...
LATEST_SESSION=$(ls -t profiles/ 2>/dev/null | head -1)
if [[ -n "$LATEST_SESSION" && -f "profiles/$LATEST_SESSION/summary.json" ]]; then
...
try:
    with open('profiles/$LATEST_SESSION/summary.json', 'r') as f:
        results = json.load(f)
...
if [[ -f "scripts/profile_harness.py" ]]; then
    echo "  Running dry-run test (max 3 examples)..."
    python3 scripts/profile_harness.py --profile all --dry-run --max-examples 3 --summary 2>/dev/null || echo "  ❌ Dry-run failed"
else
...
echo "  ./archive/clean_profiles.sh  # Clean old profiles"

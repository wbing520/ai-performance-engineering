...
import lit_gpt
from lit_gpt.config_rmsnorm_compile import Config as BaseConfig
...
from lit_gpt.utils_reduce_sync import map_old_state_dict_weights
...
class LoRAQKVLinear(LoRALinear):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if r > 0 and any(enable_lora):
            ...
            # UNCOMMENT THIS IF YOU WANT TO REMOVE EXCESSIVE cudaDeviceSynchronizations
            self.lora_ind_cache = {}
    ...
    def zero_pad(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # UNCOMMENT THIS OUT IF YOU WANT TO REDUCE cudaDeviceSynchronization!!
        lora_ind_tensor = self.lora_ind_cache.get(result.device)
        if lora_ind_tensor is None:
            lora_ind_tensor = self.lora_ind_cache[result.device] = torch.tensor(self.lora_ind, device=result.device)
        result = result.index_copy(
            1, lora_ind_tensor, x.reshape(-1, sum(self.qkv_shapes))
        )  # (4096, 256)               
        # result = result.index_copy(
...
class Config(BaseConfig):
    ...
    def mlp_class(self) -> Type:
        return getattr(lit_gpt.lora_reduce_sync, self._mlp_class)

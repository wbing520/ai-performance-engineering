...
try:
    # HQQ exposes quantization via the Quantizer class, not top-level functions
    from hqq.core.quantize import Quantizer as HQQQuantizer
    HQQ_AVAILABLE = True
except ImportError:
    HQQ_AVAILABLE = False
    print("Warning: HQQ not available. Some quantization features will be disabled.")
    print("To enable HQQ, install: pip install hqq")
...
class DynamicQuantizedCache:
    ...
    def async_quantize(self, tensor: torch.Tensor, nbits: int, 
        ...
        def task():
            # Use HQQ Quantizer API; avoid expensive optimization for demo speed
    ...
    def maybe_quantize_cache(self, layers, policy: str, 
        ...
        for layer_idx, layer in enumerate(layers):
            # Support layers that expose key/value caches directly
            for cache_type in ['key_cache', 'value_cache']:
                ...
                try:
                    if HQQ_AVAILABLE:
                        # HQQ returns (W_q, meta)
                        W_q, meta = future.result(timeout=2.0)
                        setattr(layer, f'{cache_type}_q', W_q)
                        setattr(layer, f'{cache_type}_meta', meta)
                        # Keep original attribute pointing to quantized buffer for demo purposes
                        setattr(layer, cache_type, W_q)
                    else:
                        quantized_tensor, scale = future.result(timeout=2.0)

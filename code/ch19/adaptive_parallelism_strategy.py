...
class DynamicParallelismRouter:
    ...
    def choose_worker_pool(self, seq_len: int, gpu_mem_util: float, 
        ...
        if concurrent_reqs > 32 and seq_len <= self.seq_len_threshold_short:
            return ParallelismStrategy.DATA_PARALLEL
        
        # For long contexts or high memory pressure, use hybrid/pipeline parallelism
        elif seq_len > self.seq_len_threshold_long or gpu_mem_util > self.memory_threshold_high:
            if gpu_mem_util > 0.9 or seq_len > 2048:
                # Extreme memory pressure or very long sequences - use pipeline for memory efficiency
...
def main():
    ...
    # Interactive demo (run for limited time)
    print(f"\n=== Interactive Dynamic Inference Server ===")
    print("Starting server with adaptive parallelism...")
    print("Running for 10 seconds to demonstrate adaptive behavior...")
    
    try:
        server = DynamicInferenceServer(available_gpus=8)
        server.running = True
        server.metrics_collector.start_monitoring()
        
        # Run for 10 seconds
        start_time = time.time()
        while server.running and (time.time() - start_time) < 10.0:
            try:
                # Get current metrics
                current_metrics = server.metrics_collector.collect_current_metrics()
                
                # Check if we should switch strategy
                new_strategy = server.router.should_switch_strategy(current_metrics)
                if new_strategy:
                    config = server.router.switch_strategy(new_strategy)
                    server.strategy_switches += 1
                
                # Process requests (simulate)
                server._process_requests(current_metrics)
        ...
        # Stop the server
        server.stop()
        print(f"\nServer stopped after {time.time() - start_time:.1f} seconds")
        print(f"Processed {server.processed_requests} requests with {server.strategy_switches} strategy switches")

...
# NOTE:  YOU MUST RE-ENABLE @torch.compile IN rmsnorm.py BEFORE RUNNING!!
import os
...
from lit_gpt.lora_reduce_sync_and_rmsnorm_compile import GPT, Block, Config, lora_filter, mark_only_lora_as_trainable
...
def main(fabric: L.Fabric, devices: int, config: Config, io: IOArgs, train: TrainArgs, eval: EvalArgs, profile_only) -> None:
    ...
    # UNCOMMENT THIS TO Enable maximum autotuning for TorchInductor if desired
...
def fit(
    ...
    for iter_num in range(1, train.max_iters(devices) + 1):
        ...
        with fabric.no_backward_sync(model, enabled=is_accumulating):
            ...
            # TODO UNCOMMENT THIS TO FREE UP logits TO AVOID HUGE SPIKES WHEN LARGE SEQUENCE IS SEEN
            del logits

...
def chunked_cross_entropy(
    ...
    if isinstance(logits, list):
        ...
        # UNCOMMENT THIS OUT TO AVOID EXTRA cudaDeviceSynchronize()
        non_masked_elems = non_masked_elems.maximum(torch.ones_like(non_masked_elems))
        return torch.cat(loss_chunks).sum() / non_masked_elems
    ...
    # UNCOMMENT THIS OUT TO AVOID EXTRA cudaDeviceSynchronization
    non_masked_elems = non_masked_elems.maximum(torch.ones_like(non_masked_elems))
    return torch.cat(loss_chunks).sum() / non_masked_elems
    # return torch.cat(loss_chunks).sum() / max(1, non_masked_elems)

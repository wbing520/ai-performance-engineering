#!/bin/bash
# Normalise Makefiles for Blackwell (SM100) with CUDA 12.8 toolchain.
...
find "$SCRIPT_DIR" -name Makefile -type f | while read -r makefile; do
  ...
  sed -i "s/ARCH?=sm_90/ARCH?=sm_100/g" "$makefile"
  sed -i "s/ARCH=sm_90/ARCH=sm_100/g" "$makefile"
  sed -i "s/CUDA_VERSION = .*/CUDA_VERSION = 12.8/g" "$makefile"
  sed -i "s/-arch=sm_90/-arch=sm_100/g" "$makefile"
...
echo "Makefiles normalised for Blackwell (sm_100, CUDA 12.8)."
